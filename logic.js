...
// maintain a counter , to check if counter goes
...
function countDown(){
    interval = setInterval(function(){
        countDownValue--;
        document.getElementById('output').innerText = `Game is over & Reset After ${countDownValue}Sec`;
    },1000);
}
...
function printXorZero(){
    ...
    this.innerText= buttonValue; // <button><button>
    flag = !flag;
    if(count>=5){
            if(isGameOver()){
                blocked = true;
                document.getElementById('output').innerText = `Game is over & Reset After 5 Sec`;
                resetAfter5Sec();
                // alert("game over")
            }
    }
